...
// Intersection reveal (with unobserve to save performance)
const io = new IntersectionObserver((entries)=>{
  entries.forEach(e=>{
    if(e.isIntersecting){
      e.target.classList.add('show');
      io.unobserve(e.target); // stop observing after reveal
    }
  });
}, {threshold: .15});
...
(function particles(){
  ...
  const dots = Array.from({length: 40}, ()=> ({ // 👈 أقل من 80 → أسرع
